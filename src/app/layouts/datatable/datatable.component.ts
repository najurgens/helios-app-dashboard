...
ngOnInit(): void {
    ...
    fromEvent(this.dataSearchInput.nativeElement, 'keyup').pipe(
    // get value
    map((event: any) => {
  ...
  onSort($event: INglDatatableSort) {
    ...
    this.tableData.sort((a: any, b: any) => {
      return (key === 'rank' ? b[key].toString() - a[key] : b[key].toString().localeCompare(a[key])) * (order === 'desc' ? 1 : -1);
    });
  }
  ...
  toggleFilter(){
    ...
    (element.hasClass('slds-hide')) ? element.removeClass("slds-hide") : element.addClass("slds-hide");
  }
